...
public class DeleteContentVerfication  extends BaseScripts{
	...
	//		logger.info("===========================Test Started=======================");
			...
			for(int i=1;i<rowCount;i++)
			{
				...
				Boolean flag=connect().AddContents().verfiypresenceOfContent1();
					if(flag)
					{
						connect().AddContents().clickEdit();
						connect().AddContents().clickDelete();
						connect().AddContents().doneEditing();
					}
					
					connect().AddContents().verifyContentDeleted(editedTitle);
				}
